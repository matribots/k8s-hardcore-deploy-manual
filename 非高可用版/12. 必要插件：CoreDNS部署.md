...
- 完成 [10. nodes部署：kube-proxy](./10. nodes部署：kube-proxy.md)
- 完成 [11. 集群网络部署：Calico部署](./11. 集群网络部署：Calico部署)

&nbsp;

# 2. 部署CoreDNS

CoreDNS部署本身比较简单，通过yaml部署至k8s集群里就好，具体的配置参数请参考文档。
...
spec:
  selector:
    k8s-app: kube-dns
  clusterIP: 10.68.0.2 # 指定DNS服务的K8s service地址，注意要和kubelet启动参数中的一致！
  ports:
  - name: dns
    port: 53
    protocol: UDP
...
## 3.1. 从K8s集群内部通过域名方式Nginx service

通过观察可以发现，DNS部署成功后，能够在K8s Pod内部通过Nginx service域名的方式访问Nginx服务。

![](../pictures/curl_nginx_svc_domainname.jpg)
