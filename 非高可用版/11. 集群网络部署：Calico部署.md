为K8s集群部署Calico网络从步骤上来说是比较简单的 —— 如果仅仅是为了用Calico部署一个可用的k8s网络，只要修改对应的参数并创建对应的K8s CRD资源即可。本文档只描述部署一个可用的k8s Calico网络的部署；关于K8s网络模型、Calico的组网原理、参数调优等，需要自行去深入了解。
...
![](../pictures/curl_nodeport_nginx_svc.jpg)
